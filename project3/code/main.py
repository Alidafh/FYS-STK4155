...
from CNN import create_model, fit_model
from generate import load_data

def plot_training_data(label_names=None):
...
def plot_test_results(label_names=None):
    plt.figure(figsize=(10,10))
    for i in range(25):
        plt.subplot(5,5,i+1)
        plt.xticks([])
        plt.yticks([])
        plt.imshow(X_test[i][:,:,0], cmap="inferno")
        n = y_pred[i].argmax()
        m = y_test[i].argmax()
        if label_names:
...
history = fit_model(X_train, y_train, X_test, y_test, model, verbose=1)
