        ...
        while not check_if_stop(ep, n_epochs, mse=stop_cri, threshhold=stop_threshhold):    
            ...
            for i, (x_batch, y_batch) in enumerate(list(zip(X_batches,y_batches))):
                ...
                for k, el in enumerate(beta_temp):
                    ...
                    # step_size[k] = momentum[k]*learn_rate if learn_rate else momentum[k]*tools.learning_schedule(ep*m+i, 5, 50)
                    step_size[k] = momentum[k]*lr
                    # print("test4.5")
                    # beta_temp[k] = el - step_size[k]/len_batch - self.regularize_index(k)*self.regularization_type(el)*lamb*learn_rate/n
...
class Network(Regression):    # "(object)" in book
    ...
    def evaluate(self, test_data):
        ...
        # if test: self.accuracies.append(sum(int(x == y) for (x, y) in self.test_results)/len(test_data))
        # else: self.accuracies_train.append(sum(int(x == y) for (x, y) in self.test_results)/len(test_data))
