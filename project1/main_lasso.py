...
for j in range(d_max):
    ...
    for i in range(nlamb):
        ...
        z_train_1, z_test_1, z_fit_1, z_pred_1 = func.regression(z, X, "LASSO", lamb=lambdas[i])
        m_test[:,i] = func.metrics(z_test_1, z_pred_1, test=True)
        m_train[:,i] = func.metrics(z_train_1, z_fit_1, test=True)

        # With bootstrapping
        z_train_2, z_test_2, z_fit_2, z_pred_2 = func.Bootstrap(x, y, z, d_ridge, n_bootstraps, RegType="LASSO", lamb=lambdas[i])
        m_test_bs[:,i] = func.metrics(z_test_2, z_pred_2, test=True)
        m_train_bs[:,i] = func.metrics(z_train_2, z_fit_2, test=True)

        # With kFold
        z_train_4, z_test_4, z_fit_4, z_pred_4 = func.kFold(x, y, z, d_ridge, k, shuffle=True, RegType="LASSO", lamb=lambdas[i])
...
info_ridge1 = "n{:.0f}".format(len(z), best_lamb_ridge)
func.print_plot_modelparams(x, y, z, m_test=best_m_ridge, d =best_degree_ridge, lamb=best_lamb_ridge, rType = "LASSO", quiet = False, info=info_ridge1, f=FOLDER)
...
info_ridge_bs = "n{:.0f}_bs{:}".format(len(z), best_lamb_ridge_bs, n_bootstraps)
func.print_plot_modelparams(x, y, z, best_m_ridge_bs, best_degree_ridge_bs, best_lamb_ridge_bs, rType = "LASSO", quiet = False, info=info_ridge_bs, f=FOLDER)
...
info_ridge_k = "n{:.0f}_kFold{:}".format(len(z), best_lamb_ridge_k, k)
func.print_plot_modelparams(x, y, z, best_m_ridge_k, best_degree_ridge_k, best_lamb_ridge_k, rType = "LASSO", quiet = False, info=info_ridge_k, f=FOLDER)
...
for i in range(d_max):
    ...
    z_train_k, z_test_k, z_fit_k, z_pred_k = func.kFold(x, y, z, degrees[i], k, shuffle=True, RegType="LASSO", lamb=0.01)
    ...
    z_train_1, z_test_1, z_fit_1, z_pred_1 = func.Bootstrap(x, y, z, degrees[i], n_bootstraps, RegType="LASSO", lamb=1)
    ...
    z_train_2, z_test_2, z_fit_2, z_pred_2 = func.Bootstrap(x, y, z, degrees[i] , n_bootstraps, RegType="LASSO", lamb=0.01)
    ...
    z_train_3, z_test_3, z_fit_3, z_pred_3 = func.Bootstrap(x, y, z, degrees[i], n_bootstraps, RegType="LASSO", lamb=0.001)
...
info_r_bs3 = "n{:.0f}_bs{:}_lamb{:.4f}".format(len(z), n_bootstraps, 0.000)

plot.bias_variance(degrees, m_test_bs1[1], m_test_bs1[2], m_test_bs1[3], x_type="degrees", RegType ="LASSO", info=info_r_bs1, log=True,FOLDER=FOLDER)
plot.bias_variance(degrees, m_test_bs2[1], m_test_bs2[2], m_test_bs2[3], x_type="degrees", RegType ="LASSO", info=info_r_bs2, log=True,FOLDER=FOLDER)
plot.bias_variance(degrees, m_test_bs3[1], m_test_bs3[2], m_test_bs3[3], x_type="degrees", RegType ="LASSO", info=info_r_bs3, log=True,FOLDER=FOLDER)
...
plot.compare_MSE(degrees, m_test[1], m_test_bs2[1], m_test_k[1], rType = "LASSO", lamb=0.01, info=info_comp, log=True, FOLDER=FOLDER)
plot.compare_R2(degrees, [m_test[0], m_train[0]], [m_test_bs2[0], m_train_bs2[0]], [m_test_k[0],m_train_k[0]] , rType = "LASSO", info=info_comp, FOLDER=FOLDER)
...
for i in range(nlamb):
    ...
    # With kFold for d = 6
    z_train_k, z_test_k, z_fit_k, z_pred_k = func.kFold(x, y, z, 6, k, shuffle=True, RegType="LASSO", lamb=lambdas[i])
    m_test_k[:,i] = func.metrics(z_test_k, z_pred_k, test=True)
    m_train_k[:,i] = func.metrics(z_train_k, z_fit_k, test=True)

    # With bootstrapping for d = 2
    z_train_1, z_test_1, z_fit_1, z_pred_1 = func.Bootstrap(x, y, z, 2, n_bootstraps, RegType="LASSO", lamb=lambdas[i])
    m_test_bs1[:,i] = func.metrics(z_test_1, z_pred_1, test=True)
    m_train_bs1[:,i] = func.metrics(z_train_1, z_fit_1, test=True)

    # With bootstrapping for d = 6
    z_train_2, z_test_2, z_fit_2, z_pred_2 = func.Bootstrap(x, y, z, 6 , n_bootstraps, RegType="LASSO", lamb=lambdas[i])
    m_test_bs2[:,i] = func.metrics(z_test_2, z_pred_2, test=True)
    m_train_bs2[:,i] = func.metrics(z_train_2, z_fit_2, test=True)

    # With bootstrapping for d = 9
    z_train_3, z_test_3, z_fit_3, z_pred_3 = func.Bootstrap(x, y, z, 9, n_bootstraps, RegType="LASSO", lamb=lambdas[i])
    m_test_bs3[:,i] = func.metrics(z_test_3, z_pred_3, test=True)
    m_train_bs3[:,i] = func.metrics(z_train_3, z_fit_3, test=True)
...
plot.bias_variance(lambdas, m_test_bs1[1], m_test_bs1[2], m_test_bs1[3], x_type="lambda", RegType ="LASSO", info=info_ridge_d2, log=True,FOLDER=FOLDER)
plot.bias_variance(lambdas, m_test_bs2[1], m_test_bs2[2], m_test_bs2[3], x_type="lambda", RegType ="LASSO", info=info_ridge_d6, log=True,FOLDER=FOLDER)
plot.bias_variance(lambdas, m_test_bs3[1], m_test_bs3[2], m_test_bs3[3], x_type="lambda", RegType ="LASSO", info=info_ridge_d9, log=True,FOLDER=FOLDER)
